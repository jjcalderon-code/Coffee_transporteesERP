...
st.markdown(
    ...
    /* ============================================================
       MENÚ DE NAVEGACIÓN (radio) estilizado como lista de opciones
       ============================================================ */
...
if not st.session_state.logged_in:
  ...
  with st.sidebar:
    ...
    options_display_list = [menu_display_map[m] for m in menu_options_raw]
    with st.container(border=True):
      st.markdown("<div class='ct-nav-title'>📋 Menú</div>", unsafe_allow_html=True)
      choice_display = st.radio(
          "Navegación", options_display_list, label_visibility="collapsed"
      )

    choice = [k for k, v in menu_display_map.items() if v == choice_display][0]
